# Clothes Shop built on docker-compose-laravel
...
**Start**: [http://localhost:8080/admin/users/login](http://localhost:8080/admin/users/login)
...
1. Create a `db` folder in the project root, alongside `src` folders.
...
The version of Laravel (9) uses MailHog as the default application for testing email sending and general SMTP work during local development. Using the provided Docker Hub image, getting an instance set up and ready is simple and straight-forward. The service is included in the `docker-compose.yml` file, and spins up alongside the webserver and database services.
